...
const UploadPreOrderProduct = ({
    ...
    const handleSubmit = async (e) => {
        ...
        console.log('input error: ', Object.keys(error).length);
        if (Object.keys(error).length === 0) {

            console.log('Có vào đây không thì bảo !!!');
            const response = await fetch(SummaryApi.createPreOrderProduct.url, {
                method: SummaryApi.createPreOrderProduct.method,
                credentials: 'include',
                headers: {
            ...
            if (responseData.success) {
                ...
                onClose();
                // await fetch(SummaryApi.sendIntroductNewProductMail.url, {
                //     method: SummaryApi.sendIntroductNewProductMail.method,
                //     credentials: 'include',
                //     headers: {
                //         "content-type": "application/json"
                //     },
                //     body: JSON.stringify({ data: responseData.data }),
                // })
                fetchData();
            }
...
export default UploadPreOrderProduct;